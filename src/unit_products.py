def remove_unit_products(cfl):
    ...
    for _ in range(num_unit_products):
        ...
        # loop unit_rules' products & substitute vars from uf_rules list
        new_sub_rules = []
        for unit_rule in unit_rules:
            uf_products = []
            for unit_product in unit_rule["RHS"]:

                # for each unit-product substitute from the
                # uf_rules var's products
        ...
        updated_ufs_rules = []
        for uf_rule in uf_rules:
            matching_sub_rule = next((sub_rule for sub_rule in new_sub_rules
                                        if sub_rule["LHS"] == uf_rule["LHS"]), None)
            if matching_sub_rule:
                merged_rhs = list(set(uf_rule["RHS"] + 
                                        matching_sub_rule["RHS"]))
                updated_ufs_rules.append({"LHS": uf_rule["LHS"],
                                            "RHS": merged_rhs})
            else:
                updated_ufs_rules.append({"LHS": uf_rule["LHS"],
                                            "RHS": uf_rule["RHS"]})
